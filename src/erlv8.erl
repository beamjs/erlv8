...
valid_vm_creation_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	?assert(is_pid(Script)),
	?assertEqual({ok, 2}, erlv8_vm:run(Script,"1+1;")),
	ok = stop().
...
few_vms_test() ->
	...
	{ok, Script} = erlv8_vm:new(),
	?assertEqual({ok,2}, erlv8_vm:run(Script,"1+1;")),
	?assertEqual({ok,4}, erlv8_vm:run(Script,"2*2;")),
	stop().
...
compilation_error_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	?assertMatch({compilation_failed, _}, erlv8_vm:run(Script,"1+;")),
	stop().

script_stopping_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:stop(Script),
...
script_global_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:run(Script,"var a = 1+1;"),
	Global = erlv8_vm:global(Script),
...
js_to_term_fun_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:run(Script,"x = function () {}"),
	Global = erlv8_vm:global(Script),
...
js_object_to_term_fun_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:run(Script,"x = function () {}; x.a = 1"),
	Global = erlv8_vm:global(Script),
...
term_to_js_object_fun_erlv8_fun_test() ->
	...
	{ok, Script} = erlv8_vm:new(),
	Global = erlv8_vm:global(Script),
	{ok, {erlv8_fun,_, Script, O}=Fun} = erlv8_vm:run(Script,"x = function () {}; x.a = 1; x"),
	?assertEqual([{"a",1}],O:proplist()),
...
term_to_js_object_fun_test() ->
	...
	XObj = X:object(),
	XObj:set_value("y",1),
	?assertMatch({ok, 1}, erlv8_vm:run(Script,"x.y")),
	X0 = Global:get_value("x"), X1 = X0:object(),
	?assertMatch(1, X1:get_value("y")),
	?assertMatch({ok, 123}, erlv8_vm:run(Script,"x()")),
	stop().
...
term_to_js_error_test() ->
	...
	{exception, Exception} = erlv8_vm:run(Script,"x()"),
	?assertEqual("Hello", Exception:get_value("message")),
	Global:set_value("x",fun (#erlv8_fun_invocation{},[]) -> {throw, "Goodbye"} end),
	{exception, "Goodbye"} = erlv8_vm:run(Script,"x()"),
	stop().

object_fun_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	{ok, Fun} = erlv8_vm:run(Script,"f = function() {}; f.y = 1; f"),
	FunObj = Fun:object(),
...
invocation_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation,[]) -> 123 end, F end),
	?assertEqual({ok, 123}, erlv8_vm:run(Script,"test()")),
	stop().

fun_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:register(Script, "test0", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation, [F]) -> F:call([321]) end, F end),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation,[Val]) -> Val end, F end),
	?assertEqual({ok, 321}, erlv8_vm:run(Script,"f = function(x) { return test(x) }; test0(f);")),
	stop().

fun_server_is_pid_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{ server = Server} = _Invocation,[]) -> is_pid(Server) end, F end),
	?assertEqual({ok, true}, erlv8_vm:run(Script,"test();")),
	stop().
...
fun_returning_fun_test() ->
	...
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation,[Val]) -> Val end, F end),
	{ok, {erlv8_fun, _, Script, O}} = erlv8_vm:run(Script,"f = function() {}; test(f);"),
...
fun_new_vm_inside_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation,[]) -> {ok, _Pid} = erlv8_vm:new(), 321 end, F end),
	?assertEqual({ok, 321},erlv8_vm:run(Script, "test()")),
	stop().
...
fun_this_test() ->
	...
	{ok, Result} = erlv8_vm:run(Script,"x()"),
...
fun_callback_test() ->
	...
	{ok, Script} = erlv8_vm:new(),
	Self = self(),
	erlv8_vm:register(Script, "test", fun () -> F = fun (#erlv8_fun_invocation{} = _Invocation, [Cb]) -> 
														   spawn(fun () ->
	...
	erlv8_vm:run(Script,"f = function(x) { return x}; test(f);"),
	receive
		{ok, 1} ->
			ok;
		Other1 ->
...
to_string_test() ->
	start(),
	{ok, Script} = erlv8_vm:new(),
	?assertEqual("1",erlv8_vm:to_string(Script,1)),
	?assertEqual("1",erlv8_vm:to_string(Script,"1")),
	?assertEqual("true",erlv8_vm:to_string(Script,true)),
	?assertEqual("[object Object]",erlv8_vm:to_string(Script,[])),
	stop().
...
proto_test() ->
	...
	?assertEqual({ok, 1},erlv8_vm:run(Script,"obj.x")),
	stop().
